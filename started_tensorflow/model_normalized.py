...
def inference(image_node):
  ...
  with tf.variable_scope('conv1') as scope:
    ...
    biases = _get_biases([64], value=0.0)
...
def inference_with_dropout(image_node, keep_prob):
  # conv1
  with tf.variable_scope('conv1') as scope:
    weights = _get_weights(shape=[5, 5, 3, 64], stddev=1e-4)
    conv = tf.nn.conv2d(image_node, weights, [1, 1, 1, 1], padding='SAME')
  ...
  with tf.variable_scope('fc3') as scope:
    ...
    fc3 = tf.nn.relu(tf.matmul(fc3, weights) + biases, name=scope.name)

  # fc4
  with tf.variable_scope('fc4') as scope:
    weights = _get_weights(shape=[384, 192], stddev=0.04)
    biases = _get_biases([192], value=0.1)
    fc4 = tf.nn.dropout(fc3, keep_prob)
    fc4 = tf.nn.relu(tf.matmul(fc4, weights) + biases, name=scope.name)
