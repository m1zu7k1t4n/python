...
# ランダム加工に白色化（Whitening）を加える


FLAGS = tf.app.flags.FLAGS
tf.app.flags.DEFINE_integer('epoch', 30, "訓練するEpoch数")
tf.app.flags.DEFINE_integer('learning_rate', 0.001, "学習率")
...
def _save(sess, op, file_index, index):
  file_name = filenames[file_index]
  basename = os.path.basename(file_name)
  path = os.path.dirname(file_name)

  reader = Cifar10Reader(file_name)
  image = reader.read(index)

  img = sess.run(op, feed_dict={
    'train_image:0': image.image,
    'label:0': image.label
  })

  imageshow = Image.fromarray(img.astype(np.uint8))

  file_name = 'aa-%s-%d.png' % (basename, index)
...
def main(argv=None):
  ...
  with tf.Session() as sess:
    ...
    for epoch in range(1, FLAGS.epoch + 1):
      ...
      print('epoch %d duration = %d sec, prediction = %.2f'
